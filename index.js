...
client.on("message", async (msg) => {
    ...
    if (conversation.step === "awaiting_feedback") {
        ...
        switch (text) {
            case "1":
                ...
                replyText = "Syukurlah kalau begitu! Terima kasih feedback-nya ya. 😊 Ada lagi yang bisa dibantu?"
            ...
            case "3":
                ...
                // Tetap di step awaiting_feedback untuk menunggu jawaban 'YA'
                break
            default:
    ...
    if (conversation.step === "ask_nim") {
        ...
        await db.query("UPDATE conversations SET step = 'ask_name', nim = ? WHERE id = ?", [nim, conversation.id])
    ...
    if (conversation.step === "ask_name") {
        ...
        try {
            ...
            let extractedName = result.response.text().trim()
        ...
        await db.query("UPDATE conversations SET step = 'menu', name = ? WHERE id = ?", [name, conversation.id])
    ...
    if (conversation.step === "menu") {
        ...
        switch (text) {
            case "1":
                ...
                break;
            default:
                const welcomeBackText = `Halo lagi, ${conversation.name}! Saya admin akademik Fakultas Ekonomi dan Bisnis Telkom University. Senang bisa bantu kamu lagi. 😊`
...
app.listen(3001, () => {
    console.log("Server walk in  http://localhost:3001");
})